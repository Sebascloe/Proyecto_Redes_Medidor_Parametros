...
class App:
    def __init__(self, master):
        ...
        self.stopped_measurement = False  # Nueva variable para rastrear si la medición está detenida
        self.stop_button_clicks = 0  # Variable para contar el número de clics en el botón "Parar"
        ...
        self.progressbar.stop()  # Detener la animación inicialmente
        ...
        button_frame_additional = tk.Frame(master)
        button_frame_additional.pack(pady=10)
        
        # Crear un botón para detener la medición en bucle
        self.stop_button = ttk.Button(button_frame_additional, text="Detener", command=self.stop_measurement_loop, style='AccentButton.TButton')
    ...
    def start_measurement(self):
        ...
        # Mostrar la barra de carga y comenzar la animación
    ...
    def stop_measurement_loop(self):
        # Incrementar el contador de clics en el botón "Parar"
        self.stop_button_clicks += 1

        # Si el número de clics es impar, detener la medición en bucle
        if self.stop_button_clicks % 2 != 0:
            self.stopped_measurement = True
            self.master.after_cancel(self.start_measurement_periodic)
            self.progressbar.stop()
            self.error_label.config(text="Medición detenida", fg="black")
        else:
            # Si el número de clics es par, reanudar la medición en bucle
            self.stopped_measurement = False
            self.start_measurement_periodic()
    
    def start_measurement_periodic(self):
        # Llamada a la función de medición solo si no se ha detenido
        if not self.stopped_measurement:
    ...
    def perform_measurement(self):
        ...
        packet_loss = 0  # Inicializar con 0 por defecto
